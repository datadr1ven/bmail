document.addEventListener("DOMContentLoaded", function () {
  ...
  fileInput.addEventListener("change", function (event) {
    ...
    if (invalidFiles.length > 0) {
      alert("Only image files are allowed. Please select images only.");
      return;
    }
    uploadedFiles = files;
    fileOrders = uploadedFiles.map((_, index) => index + 1);
    console.log("Images uploaded:", uploadedFiles.length);
  ...
  function displayOrderControls() {
    ...
    sortedFiles.forEach((file) => {
      ...
      html += `
        <div class="flex flex-col items-center p-2 bg-white rounded shadow ${isTouchDevice ? "cursor-pointer" : "cursor-move"}" ${isTouchDevice ? "" : 'draggable="true"'} data-index="${originalIndex}" style="transition: background-color 0.2s;">
        ...
        .then((items) => {
          ...
          <head>
            ...
            <style>
              ...
              .mosaic-item img { display: block; margin: 0 auto; width: 400px; height: auto; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); }
            </style>
            <!--[if mso]>
            <style>
